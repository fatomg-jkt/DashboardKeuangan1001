...
function readInitialData() {
  try {
    ...
    return Array.isArray(parsed) && parsed.length ? parsed.map(calcRow) : sampleBudgetData.map(calcRow);
  } catch {
...
function BarChart({ data, title }) {
  const max = Math.max(1, ...data.flatMap((item) => [item.budget || 0, item.actual || 0]));
  return h("div", { className: "chart-wrap", "aria-label": title }, data.length ? data.map((item) => h("div", { className: "chart-group", key: item.name }, h("div", { className: "bar-pair" }, h("span", { className: "bar budget", style: { height: `${Math.max(4, (item.budget / max) * 100)}%` } }), h("span", { className: "bar actual", style: { height: `${Math.max(4, (item.actual / max) * 100)}%` } })), h("small", null, item.name))) : h("p", { className: "empty" }, "Belum ada data untuk grafik."), h("div", { className: "legend" }, h("span", null, h("i", { className: "dot budget-dot" }), "Budget"), h("span", null, h("i", { className: "dot actual-dot" }), "Actual")));
}
function DonutList({ data }) {
  const total = data.reduce((sum, item) => sum + item.actual, 0);
  return h("div", { className: "donut-list" }, data.length ? data.map((item, index) => h("div", { className: "donut-row", key: item.name }, h("span", { className: `slice slice-${index % 5}` }), h("strong", null, item.name), h("em", null, total === 0 ? "0%" : percent((item.actual / total) * 100)), h("small", null, rupiah(item.actual)))) : h("p", { className: "empty" }, "Belum ada data departemen."));
}
function Dashboard({ rows }) {
  const summary = makeSummary(rows);
  const monthly = Object.values(groupBy(rows, "bulan")).sort((a, b) => monthOrder.indexOf(a.name) - monthOrder.indexOf(b.name));
  const departments = Object.values(groupBy(rows, "departemen"));
...
function LaporanBudgeting({ rows, setRows }) {
  ...
  const departments = [...new Set(rows.map((row) => row.departemen).filter(Boolean))];
  const filteredRows = rows.filter((row) => (filters.bulan === "Semua" || row.bulan === filters.bulan) && (filters.departemen === "Semua" || row.departemen === filters.departemen) && `${row.kategori} ${row.departemen}`.toLowerCase().includes(filters.search.toLowerCase()));
  const submit = (event) => {
    ...
    const nextRows = editingId ? rows.map((row) => (row.id === editingId ? clean : row)) : [...rows, clean];
  ...
  const remove = (id) => { const nextRows = rows.filter((row) => row.id !== id); setRows(nextRows); saveData(nextRows); };
